from automatic_prompt_engineer.automatic_prompt_engineer import llm, data, evaluate
import numpy as np
    ...
    def _compute_avg_likelihood(self, prompts, log_probs, num_samples):
        ...
        for prompt in prompts:
            ...
            for _ in range(num_samples):
                lps = log_probs[i]
                if len(lps) > 0:  # Add this check
                    prompt_log_probs[-1].append(sum(lps) / len(lps))
                else:
                    prompt_log_probs[-1].append(0)  # Append 0 or any other value that makes sense in the context of your program
                i += 1
